...
module.exports = function (props, callback) {
  ...
  function doCallback (err, res) {
    if (!complete) {
      complete = true;
      typeof callback === 'function' && callback (err, res || null);
    }
  }
  ...
  var body = typeof props.fields === 'object' ? querystring.stringify (props.fields) : null;
  ...
  request.on ('response', function (response) {
    ...
    response.on ('end', function () {
      var error = null;
      data = new Buffer.concat (data, size).toString ();
