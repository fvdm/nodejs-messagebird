...
function doRequest (request, response) {
  if (settings.whitelist && settings.whitelist.indexOf (request.socket.remoteAddress) < 0) {
  ...
  switch (url.pathname) {
    case settings.pathHLR:
      server.emit ('hlr', query);
      break;

    case settings.pathMessage:
      server.emit ('message', query);
      break;

    case settings.pathVMN:
...
*/
  http.listen (settings.port, settings.iface);
};
